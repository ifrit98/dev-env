...
try:
    ...
    warn("Could not import matplotlib.pyplot as plt.")

try:
    import numpy as np
    from numpy import array, asarray
    from numpy import mod, mean, median, var, std
    from numpy import argmax, argmin, gcd, lcm, sqrt, sin, cos
    from numpy import log, log10, log2, exp, power
except ImportError:
    warn("Could not import numpy.")
...
def get_subdirs(dirpath=None):
  get = lambda x, l: np.asarray(x)[np.where(l)]
  dirs = lambda path: [x[0] for x in os.walk(path)]
  subdirs = lambda: np.asarray(dirs('./'))
  if dirpath is None:
    sd1 = subdirs()[1:]
  else:
    sd1 = np.asarray(dirs(dirpath))[1:]
  lengths = np.asarray(lmap(sd1, lambda x: len(x.split('/'))))
  sd2 = get(sd1, lengths == min(lengths))
  sd3 = get(sd2, lmap(sd2, lambda x: x[2] != '.'))
  return sd3
...
try:
    ...
    warn("pynvml could not be loaded.")
    DEV_COUNT = 0
MB = 1024 * 1024
...
try:
    ...
    warn("Could not configure GPU devices for tensorflow...")
...
if False:
    ...
    try:
        ...
        warn("Error loading IPython utils...")
        warn("python:", sys.version)
        pass
...
maybe_list = lambda x: x if hasattr(x, "__len__") else [x]
rmatrix = lambda x: tf.random.normal(maybe_list(x)).numpy()
...
def idx_of(vals_to_idx, arr_to_seek):
    ...
    vals_to_idx = maybe_list_np(vals_to_idx)
...
def idx_of2(arr_to_seek, vals_to_idx):
    if isinstance(vals_to_idx[0], str):
...
def idx_of_str2(arr_to_seek, vals_to_idx):
    vals_to_idx = maybe_list_up(vals_to_idx)
    arr_to_seek = np.asarray(arr_to_seek)
    nested_idx = list(
        map(
...
def random_ints(length, lo=-1e4, hi=1e4):
    return [pyrand.randint(lo, hi) for _ in range(length)]

def random_floats(length, lo=-1e4, hi=1e4):
    return [pyrand.random() for _ in range(length)]
...
index2 = lambda arr, elem: unlist(logical2idx(where(arr, elem)))
...
fetch = lambda f, x: x[logical2idx(lmap(x, f))]
...
shuffle0 = lambda p: pyrand.sample(p, k=len(p))
